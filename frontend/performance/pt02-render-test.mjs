...
async function run() {
  ...
  // Prime app with auth and then inject route state for customization page.
  ...
  for (let i = 0; i < iterations; i += 1) {
    // Switch back to 2D before each measurement to keep a consistent start point.
    ...
    // Wait until second canvas appears (3D canvas mount complete).
  ...
  console.log(JSON.stringify({
    metric: 'PT-02',
    samples_ms: samplesMs.map((n) => Number(n.toFixed(2))),
...
run().catch((err) => {
  console.error('PT-02 run failed:', err);
  process.exit(1);
});
